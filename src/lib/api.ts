...
const resolveApiBaseUrl = (): string => {
  const envUrl = ((import.meta.env.VITE_BACKEND_API_URL || (import.meta.env as any).NEXT_PUBLIC_BACKEND_API_URL || '') as string).trim();
...
class ApiClient {
  ...
  async getPublicPlans(): Promise<CommercialPlanItem[]> {
    ...
    // Try through authenticated request client
    try {
    ...
    // Fallback directly to Supabase if configured
    try {
      ...
      if (!error && data && data.length > 0) {
        return data.map((item: any) => ({
          ...
          creditAllocation: item.credit_allocation,
          humanSeatLimit: item.human_seat_limit,
          aiAgentLimit: item.ai_agent_limit,
          isPriceVisible: item.is_price_visible,
    ...
    throw new Error('Failed to fetch public plans from all backend endpoints');
  }
