...
const MainDashboardApp: React.FC = () => {
  ...
  const getScreenTitle = () => {
    switch (currentScreen) {
      case 'overview':
        return { title: 'Platform Analytics & Intelligence (Fase 110)', subtitle: 'SSOT cross-tenant revenue, usage credit, token consumption, dan evaluasi Human vs AI' };
      case 'system_monitoring':
        return { title: 'System Monitoring Center & Health Gate (Fase 102 & 90)', subtitle: 'Telemetri pod Kubernetes, latensi LLM circuit breaker, antrean DLQ, dan security gate' };
      case 'tenant_workforce':
        return { title: 'Cross-Tenant Workforce & Department Telemetry (Fase 91.A, H)', subtitle: 'Visibilitas adopsi 14 divisi bisnis, distribusi 15 AI Job Titles, dan rasio Human-to-AI' };
      case 'prospect_registrations':
        return { title: 'Prospect Registrations & 36 Trial Allocation (Fase 127)', subtitle: 'Pendaftar kuesioner publik, seleksi 36 slot trial 7 hari, aktivasi tenant trial 1,000 credit, dan penjadwalan demo' };
      case 'commercial_plans':
        return { title: 'Commercial Plans & Feature Entitlements (Fase 114.1)', subtitle: 'Manajemen paket komersial, harga publik, limit seat/agent, dan matriks hak akses' };
      case 'credit_metering':
        return { title: 'Credit Metering & Multiplier Rules (Fase 114.2)', subtitle: 'Konfigurasi base work unit dan faktor pengali (complexity, model, tool, execution)' };
      case 'tenant_credit_override':
        return { title: 'Tenant Credit Wallet & Override (Fase 114.3)', subtitle: 'Inspeksi saldo wallet, manual credit adjustment ber-audit trail, dan ledger riwayat' };
      case 'financial_command_center':
        return { title: 'Financial Command Center (Fase 114.4)', subtitle: 'Ringkasan MRR, ARR, peredaran kredit, margin kotor, dan peringkat konsumsi tenant' };
      case 'payment_reconciliation':
        return { title: 'Payment Reconciliation & Anomaly Review (Fase 110)', subtitle: 'Rekonsiliasi transaksi gateway vs database lokal, deteksi order stuck, dan manual override Super Admin' };
      case 'dead_letter_queue':
        return { title: 'Dead-Letter Queue & Workflow Replay (Fase 109)', subtitle: 'Pemulihan kegagalan idempotensi, inspeksi error payload, dan replay eksekusi' };
      case 'workflow_tracing':
        return { title: 'Workflow Tracing & Observability (Fase 107)', subtitle: 'OpenTelemetry span per node, visualisasi durasi, dan kalibrasi confidence' };
      case 'memory_management':
        return { title: 'Memory Management & Decay (PRD 17.2)', subtitle: 'Consolidator threshold audit, linear & rolling decay, serta hybrid search re-ranking' };
      case 'tenants':
        return { title: 'Tenant Management', subtitle: 'Manajemen isolasi data organisasi dan alokasi resource' };
      case 'llm_providers':
        return { title: 'LLM Providers (Fase 93.A)', subtitle: 'Konfigurasi provider AI, latensi, dan failover sequence' };
      case 'mcp_tools':
        return { title: 'MCP Tool Registry (Fase 93.B)', subtitle: 'Model Context Protocol sandbox functions & risk gating' };
      case 'app_registry':
        return { title: 'Third-Party App Registry (Fase 93.C)', subtitle: 'Konektor OAuth, ERP, CRM, dan gateway marketplace' };
      case 'master_data':
        return { title: 'Master Data & Industry Presets (Fase 91)', subtitle: 'Template industri, default role, dan prompt guardrails' };
      case 'skill_plugins':
        return { title: 'Skill Plugin Registry (Fase 92.C)', subtitle: 'Marketplace WASM & Python sandbox extension plugins' };
      case 'usage_cost':
