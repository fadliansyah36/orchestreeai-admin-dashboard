...
export const DeadLetterAndReplayScreen: React.FC = () => {
  ...
  const handleSimulateFailedJob = async (jobName: string) => {
    setIsLoadingDlq(true);
    setActionMessage(null);
    try {
      await api.triggerSchedulerJob(jobName, 'tenant-admin-test', true);
      setActionMessage({
        type: 'success',
        text: `Simulasi 3x retry gagal untuk job ${jobName} berhasil masuk ke Dead-Letter Queue!`,
      });
      await loadDlq();
    } catch (err: any) {
      setActionMessage({
        type: 'error',
        text: `Gagal trigger simulasi job: ${err.message}`,
      });
    } finally {
      setIsLoadingDlq(false);
    }
      ...
      {activeTab === 'dlq' && (
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-900/60 p-4 rounded-xl border border-slate-800">
            ...
            {/* Quick Test Failure Generator */}
            <div className="flex items-center gap-2">
              <span className="text-xs text-slate-400">Simulasikan Job Gagal (3x retry):</span>
              <button
                onClick={() => handleSimulateFailedJob('PROACTIVE_DAILY')}
                disabled={isLoadingDlq}
                className="px-2.5 py-1 text-xs bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 rounded-lg transition"
              >
                + Proactive Daily
              </button>
              <button
                onClick={() => handleSimulateFailedJob('COMPETITOR_CRAWL')}
                disabled={isLoadingDlq}
                className="px-2.5 py-1 text-xs bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 rounded-lg transition"
              >
                + Competitor Crawl
              ...
              <button
                onClick={() => handleSimulateFailedJob('HEALTH_CHECK')}
                disabled={isLoadingDlq}
                className="px-2.5 py-1 text-xs bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 rounded-lg transition"
              >
                + Health Check
                    ...
                    <tr>
                      <td colSpan={6} className="py-8 text-center text-slate-500 font-sans">
                        {isLoadingDlq
                          ...
                          : 'Tidak ada item di Dead-Letter Queue. Semua job berjalan normal atau gunakan tombol simulasi di atas.'}
                      </td>
                    </tr>
                  ) : (
                    dlqItems.map((item) => (
        ...
        <div className="space-y-6">
          ...
          <div className="p-4 rounded-xl bg-teal-950/30 border border-teal-800/60 flex items-start gap-3">
            ...
            <div className="text-xs text-slate-300 leading-relaxed">
              <span className="font-semibold text-teal-300">Deterministic Replay Sandbox Mode (Master PRD 20.2):</span>{' '}
