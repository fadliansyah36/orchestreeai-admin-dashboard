...
export const PaymentReconciliationScreen: React.FC = () => {
  ...
  const handleSimulateBlockedWebhookAnomaly = async () => {
    setIsSimulating(true);
    setActionError(null);
    try {
      const sim = await api.simulateStuckPaymentAnomaly();
      await fetchData(true);
      setActiveTab('review');
      setAuditSuccessBanner({
        queueId: sim.orderId,
        orderId: sim.orderId,
        superAdminId: 'SANDBOX-SIMULATION',
        reason: `Transaksi anomali berhasil disimulasikan (Order ${sim.orderId} stuck 15 min, webhook diblokir, amount mismatch). Muncul di tab Perlu Review.`,
        timestamp: Date.now(),
      });
    } catch (err: any) {
      setActionError(`Gagal mensimulasikan anomali: ${err.message}`);
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/90 border border-slate-800 p-5 rounded-2xl backdrop-blur-sm">
        ...
        {/* Action Buttons: Simulate Stuck Webhook & Auto-Check */}
        <div className="flex flex-wrap items-center gap-2.5">
          <button
            onClick={handleSimulateBlockedWebhookAnomaly}
            disabled={isSimulating}
            className="flex items-center space-x-2 px-3.5 py-2 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 border border-amber-500/30 text-xs font-semibold transition-all disabled:opacity-50"
            title="Simulasi skenario DoD: webhook gateway sengaja terblokir dan nominal mismatch"
          >
            <Sparkles className={`w-3.5 h-3.5 ${isSimulating ? 'animate-spin' : ''}`} />
            <span>{isSimulating ? 'Menyiapkan Anomali...' : 'Simulasi Webhook Diblok (DoD)'}</span>
          </button>
        ...
        <div className="flex items-center space-x-3">
          ...
          {realtimeCount > 0 && (
            <span className="text-slate-500 text-[11px]">
              ({realtimeCount} event terdeteksi via Supabase stream)
            </span>
          )}
        </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-3">
        ...
        <div className="flex items-center space-x-3">
          ...
          >
            ...
            <option value="tenant-enterprise-001">PT Nusantara Energy</option>
            <option value="tenant-growth-002">CV Retail Sukses</option>
            <option value="tenant-scale-003">Kopi Nusantara Co</option>
          </select>
        </div>
      </div>
          ...
          {filteredQueueItems.length === 0 ? (
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-12 text-center">
              ...
              <p className="text-xs text-slate-400 max-w-md mx-auto mt-1 mb-5">
                Semua transaksi berhasil tersinkronisasi atau telah diselesaikan. Ingin menguji skenario DoD dengan webhook sandbox diblokir?
              </p>
              <button
                onClick={handleSimulateBlockedWebhookAnomaly}
                disabled={isSimulating}
                className="inline-flex items-center space-x-2 px-4 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 text-white text-xs font-bold transition-all shadow-md shadow-amber-950/40"
              >
                <Sparkles className="w-4 h-4" />
                <span>Simulasikan Webhook Diblok Sekarang (DoD Test)</span>
              </button>
            </div>
          ) : (
            <div className="space-y-4">
                    ...
                    <div className="bg-slate-950/60 border border-rose-900/40 rounded-xl p-4">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-bold uppercase tracking-wider text-rose-400 flex items-center gap-1.5">
                          ...
                          Status Database Lokal (Client / Supabase)
                        </span>
          ...
          <div className="bg-slate-900 border border-slate-700 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleConfirmOverride} className="p-5 space-y-4">
              ...
              <div className="bg-purple-950/30 border border-purple-800/60 p-3 rounded-xl text-xs text-purple-200 flex items-start space-x-2.5">
                ...
                <div>
                  <p className="font-bold text-purple-300">Jejak Audit Kekal (PRD Master 25.1):</p>
