      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-lg">
          ...
          <p className="text-2xl font-bold text-white mt-3 font-mono">
            {isLoading ? '...' : analytics?.totalTokens.toLocaleString() || '4,528,900'}
          </p>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-lg">
          ...
          <p className="text-2xl font-bold text-emerald-400 mt-3 font-mono">
            ${isLoading ? '...' : analytics?.totalCostUsd.toFixed(2) || '12.84'}
          </p>
          <div className="flex items-center text-[11px] text-emerald-400 mt-2 font-medium">
            <ArrowUpRight className="w-3.5 h-3.5 mr-1" />
            <span>94.2% Cost efficiency vs Direct API</span>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-lg">
          ...
          <p className="text-2xl font-bold text-white mt-3 font-mono">$0.0028</p>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-950/60 border-b border-slate-800 text-[11px] font-semibold text-slate-400 uppercase tracking-wider">
              ...
              <th className="py-3 px-6 text-right">Effective Margin</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-800/80 text-xs">
            {(analytics?.breakdown || [
              { tenant: 'tenant-enterprise-001', tokens: 3120000, costUsd: 9.20 },
              { tenant: 'tenant-growth-002', tokens: 1408900, costUsd: 3.64 }
            ]).map((b) => (
              <tr key={b.tenant} className="hover:bg-slate-800/40 transition">
                ...
                <td className="py-3.5 px-6 text-right font-semibold text-slate-300">82.5%</td>
              </tr>
            ))}
          </tbody>
        </table>
